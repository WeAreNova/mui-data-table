import { alpha, css, SxProps, TableRow, Theme } from "@mui/material";
...
const BodyRow = <RowType extends BaseData, AllDataType extends RowType[]>({
  ...
  const tableRowStyles = useMemo<SxProps<Theme>>(
    () => [
      !isAlternateRowColour ? {} : (theme) => ({ backgroundColor: alpha(theme.palette.error.dark, 0.9) }),
      !isDisabledRow
        ? {}
        : (theme) => ({
      ...
      !rowsSelectable || !selectedRows[rowId] ? {} : (theme) => ({ backgroundColor: theme.palette.action.selected }),
    ],
    [isAlternateRowColour, isDisabledRow, rowId, rowsSelectable, selectedRows],
  );

  const onHover = useCallback<MouseEventHandler<HTMLTableRowElement>>(
    (e) => {
      if (!rowClick) return;
      e.currentTarget.classList.add(rowHoverClass.name);
      ...
      for (let row = e.currentTarget.previousSibling as Element; row; row = row.previousSibling as Element) {
        ...
        row.querySelectorAll("td[rowspan]").forEach((cell) => {
          ...
          if (rowSpan > rowsBetween) cell.classList.add(rowHoverClass.name);
        });
      }
    },
    [rowClick],
  ...
  const onUnHover = useCallback<React.MouseEventHandler<HTMLTableRowElement>>(
    (e) => {
      if (!rowClick) return;
      e.currentTarget.classList.remove(rowHoverClass.name);
      for (let row = e.currentTarget.previousSibling as Element; row; row = row.previousSibling as Element) {
        row.querySelectorAll("td[rowspan]").forEach((cell) => cell.classList.remove(rowHoverClass.name));
      }
    },
    [rowClick],
  );
  ...
  return (
    <TableRow key={rowId} data-testid="tableRow" onMouseOver={onHover} onMouseOut={onUnHover} sx={tableRowStyles}>
